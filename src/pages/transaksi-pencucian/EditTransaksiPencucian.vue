	...
	<v-main>
		...
		<div class="pages--container">
			<div class="page-title-container mb-4">
				<h1 class="pages--title">Ubah Transaksi Pencucian</h1>
			</div>
					...
					<v-stepper-window-item value="2">
						<v-card>
							...
							<v-card-actions>
								...
								<v-spacer></v-spacer>
								<v-btn class="primary--btn" :loading="btnLoading" @click="updateData">Simpan</v-btn>
							</v-card-actions>
						</v-card>
					</v-stepper-window-item>
				</v-stepper-window>
	...
	methods: {
		...
		async axioData() {
			try {
				...
				if (response.status == 200) {
					...
					let data = JSON.parse(JSON.stringify(response.data.data.karyawan_pencucis));
					data.forEach((item) => {
						this.list.selectedPencuci.push(item.id);
					});
				}
			} catch (error) {
				console.log(error);
		...
		async updateData() {
			try {
				...
				var url = 'api/transaksi-pencucian/' + this.id;
	...
	watch: {
		'form.kendaraan_id'(val) {
			this.kendaraan_list.forEach((item) => {
				if (item.id == val) {
					// kalo id kendaraan sm baru diganti, krn diawal bisa saja ada perbedaan harga saat proses create
					if (item.id == this.form.kendaraan_id) {
						this.form.tarif_kendaraan = item.harga;
					}
					this.form.jenis_kendaraan = item.tipe;
				}
			});
		},
	},
